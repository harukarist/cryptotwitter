...
// ユーザーのTwitter認証状態を管理するauth_twitterモジュール
...
const getters = {
  ...
  username: state => state.user ? state.user.twitter_id : '', //Twitterアカウント名を格納（nullの場合は空文字を返す）
  avatar: state => state.user ? state.user.avatar : '', //TwitterアバターURLを格納（nullの場合は空文字を返す）
}
...
const actions = {
  ...
  async register(context, data) {
    ...
    // 非同期処理でサーバーのAPIを呼び出し
    const response = await axios.post('/api/auth/twitter', data)
  ...
  async logout(context) {
    ...
    context.commit('setApiStatus', null)
    // サーバーのAPIを呼び出し
    const response = await axios.post('/api/auth/twitter/logout')
  ...
  async currentUser(context) {
    ...
    context.commit('setApiStatus', null)
    // サーバーのAPIを呼び出し
    const response = await axios.get('/api/auth/twitter/user')
